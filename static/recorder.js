...
// Last time updated: 2017-12-13 3:29:27 PM UTC
...
function benchmark_handler(event){
    var $obj = $(event.currentTarget);
    if ($obj.data('audio').trim() === '') {
        alert('audio file don\'t found');
    }
    else if (typeof +$obj.data('start') === 'number' && typeof +$obj.data('stop') === 'number') {
        play_interval(document.getElementById($obj.data('audio')), $obj.data('start'), $obj.data('stop'));
    } else {
        alert('end or start position are not a number');
    }
}

function play_interval(audio, start, stop) {
    if (!audio) {
    ...
    var state = 'ready';
    if (play_interval.audio) {
        play_interval.audio.pause();
        play_interval.audio.currentTime = 0;
    }
    clearInterval(play_interval.handler);
    play_interval.audio = audio;
    play_interval.audio.currentTime = start;
    play_interval.handler = setInterval(l, 1);

    function l () {
        if (state==='ready') {
            console.log('playing: '+audio.currentTime);
            audio.play();
            state = 'playing';
        }
        if (state === 'playing' && stop !== 0 && audio.currentTime >= stop) {
            console.log('stop: '+audio.currentTime+' ordered: '+start+' - '+stop);
            clearInterval(play_interval.handler);
            audio.pause();
